...
[TestClass]
public sealed class DateGeneratorTests
{
    ...
    [TestMethod]
    [DataRow("internal sealed partial class")]
    [DataRow("public partial class")]
    [DataRow("internal static partial class")]
    [DataRow("public abstract partial class")]
    ...
    public void Produces_nothing_when_no_class_is_marked()
    {
        ...
        run.AssertCompiles();
        Assert.IsFalse(run.Has("TelaniSourceGeneratorBuildDate.g.cs"));
    }

    // --- known bugs --------------------------------------------------------------------------

    [TestMethod]
    [Ignore("Audit finding 1: the hint names are constants but the output is registered per target rather than over Collect(), so a second marked class in the same compilation throws ArgumentException ('hintName must be unique') and BOTH classes lose GetBuildDate.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 17: only ContainingNamespace and Name are used, so the generated partial is emitted at namespace scope. That compiles - as a second, unrelated Demo.BuildDate type - while the nested class the attribute was put on never gets GetBuildDate.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 18: className.Name drops the type parameter list, so the generated partial declares a separate non-generic Demo.BuildDate. That compiles, but BuildDate<T> never gains the method.")]
    ...
    [TestMethod]
    [Ignore("Audit finding, and a TODO already in DateGenerator.Initialize: a class that forgets 'partial' gets CS0260 pointing at the user's own declaration. The generator should report a diagnostic naming the attribute instead.")]
