...
public sealed class ConfigGeneratorTests
{
    ...
    public void Generates_an_interface_and_a_Reload_method_that_compile()
    {
        ...
        run.AssertCompiles();
        Assert.IsTrue(run.Has("TelaniSourceGeneratorIAppSettings.g.cs"));
        Assert.IsTrue(run.Has("TelaniSourceGeneratorAppSettings.g.cs"));
    }
    ...
    public void Produces_nothing_when_no_class_is_marked()
    {
        ...
        run.AssertCompiles();
        Assert.IsFalse(run.Has("TelaniSourceGeneratorIAppSettings.g.cs"));
    }

    // --- known bugs --------------------------------------------------------------------------

    [TestMethod]
    [Ignore("Audit finding 8: Reload always emits '_extraStuff = store._extraStuff;', a member name hard-coded in ProduceSource. A settings class without it fails with CS0103 and no explanation.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 9: the attribute is matched by the literal name 'SettingsIgnore', so the long form [SettingsIgnoreAttribute] is ignored and the property leaks onto the interface.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 9: same name check, so [SettingsReadOnlyAttribute] is ignored and the property wrongly gets a setter on the interface.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 10: ExtractInfo only walks the members of the declaration carrying the attribute, so properties declared in the other files of a partial class are silently missing from the interface.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 11: the property type is copied as raw syntax via dec.Type.GetText(), which loses the using directives and aliases of the declaring file. The symbol's fully qualified display string is what is wanted.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 12: every property is treated as an instance property with a setter, so a static, expression-bodied or init-only property produces an uncompilable Reload (CS0176, CS0200, CS8852).")]
    ...
    [TestMethod]
    [Ignore("Audit finding 7: ProduceSource takes the namespace from settingsClasses.First() and merges every marked class's properties into one AppSettings/IAppSettings pair, so a second marked class produces CS0103/CS1061.")]
    ...
    [TestMethod]
    [Ignore("Audit finding 16: ContainingNamespace with SymbolDisplayGlobalNamespaceStyle.Omitted yields an empty string for a top-level class, so the generator emits 'namespace ;' (CS1001).")]
